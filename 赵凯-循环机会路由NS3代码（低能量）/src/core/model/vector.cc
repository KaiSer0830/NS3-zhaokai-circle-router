...
double CalculateDistance(const Vector3D &a, const Vector3D &b) {			//首尾相连计算节点距离法
	if(a.x < b.x) {
		Vector temp(a.x + 0.5, a.y, a.z);
		if ((a - b).GetLength() > (temp - b).GetLength()) {
			return (temp - b).GetLength();
		} else {
			return (a - b).GetLength();
		}
	} else {
		Vector temp(b.x + 0.5, b.y, b.z);
		if ((b - a).GetLength() > (temp - a).GetLength()) {
			return (temp - a).GetLength();
		} else {
