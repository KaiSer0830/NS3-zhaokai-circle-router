...
TypeId ByteTagIterator::Item::GetTypeId(void) const {
	return m_tid;
}

uint32_t ByteTagIterator::Item::GetStart(void) const {
	return m_start;
}

uint32_t ByteTagIterator::Item::GetEnd(void) const {
	return m_end;
}
...
TypeId PacketTagIterator::Item::GetTypeId(void) const {
	return m_data->tid;
}
...
Packet::Packet() : m_buffer(), m_byteTagList(), m_packetTagList(),
	 ...
	 * global UID
	 */
	m_metadata(static_cast<uint64_t>(Simulator::GetSystemId()) << 32 | m_globalUid, 0), m_nixVector(0) {
	m_globalUid++;
}

Packet::Packet(const Packet &o) : m_buffer(o.m_buffer), m_byteTagList(o.m_byteTagList), m_packetTagList(o.m_packetTagList), m_metadata(o.m_metadata) {
...
Packet::Packet(uint32_t size) : m_buffer(size), m_byteTagList(), m_packetTagList(),
	/* The upper 32 bits of the packet id in
	 * metadata is for the system id. For non-
	 * distributed simulations, this is simply
	 * zero.  The lower 32 bits are for the
	 * global UID
	 */
	m_metadata(static_cast<uint64_t>(Simulator::GetSystemId()) << 32 | m_globalUid, size), m_nixVector(0) {
	m_globalUid++;
}

Packet::Packet(uint8_t const *buffer, uint32_t size, bool magic) : m_buffer(0, false), m_byteTagList(), m_packetTagList(), m_metadata(0, 0), m_nixVector(0) {
	NS_ASSERT(magic);
...
Packet::Packet(const Buffer &buffer, const ByteTagList &byteTagList,
		...
		m_buffer(buffer), m_byteTagList(byteTagList), m_packetTagList(
				packetTagList), m_metadata(metadata), m_nixVector(0) {
}
...
Ptr<NixVector> Packet::GetNixVector(void) const {
	return m_nixVector;
}
...
void Packet::Print(std::ostream &os) const {
	...
	while (i.HasNext()) {
		...
		if (item.isFragment) {
			...
			os << " Fragment [" << item.currentTrimedFromStart << ":" << (item.currentTrimedFromStart + item.currentSize) << "]";
		} else {
			switch (item.type) {
			case PacketMetadata::Item::PAYLOAD:
...
uint32_t Packet::Deserialize(const uint8_t *buffer, uint32_t size) {
	...
	uint32_t metadataDeserialized = m_metadata.Deserialize(reinterpret_cast<const uint8_t*>(p), metaSize);
	...
	uint32_t bufferDeserialized = m_buffer.Deserialize(reinterpret_cast<const uint8_t*>(p), bufSize);
