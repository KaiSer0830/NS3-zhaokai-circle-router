...
TypeId GaussMarkovMobilityModel::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::GaussMarkovMobilityModel").SetParent<MobilityModel>().SetGroupName("Mobility").AddConstructor<GaussMarkovMobilityModel>().AddAttribute(
					"Bounds", "Bounds of the area to cruise.",BoxValue(Box(-100.0, 100.0, -100.0, 100.0, 0.0, 100.0)),
					MakeBoxAccessor(&GaussMarkovMobilityModel::m_bounds),
					MakeBoxChecker()).AddAttribute("TimeStep",
					"Change current direction and speed after moving for this time.",
					TimeValue(Seconds(1.0)),
					MakeTimeAccessor(&GaussMarkovMobilityModel::m_timeStep),
					MakeTimeChecker()).AddAttribute("Alpha",
					...
					MakeDoubleChecker<double>()).AddAttribute("MeanVelocity",
					...
					MakePointerChecker<RandomVariableStream>()).AddAttribute("NormalVelocity", "A gaussian random variable used to calculate the next velocity value.",
					...
					MakePointerChecker<NormalRandomVariable>()).AddAttribute("NormalDirection",
					...
					MakePointerChecker<NormalRandomVariable>()).AddAttribute("NormalPitch", "A gaussian random variable used to calculate the next pitch value.",
					StringValue("ns3::NormalRandomVariable[Mean=0.0|Variance=1.0|Bound=10.0]"),
...
void GaussMarkovMobilityModel::Start(void) {
	if (m_meanVelocity == 0.0) {					//如果没有设置平均速度，则获取一个随机值
	...
	//Vector position1 = m_helper.GetCurrentPosition();
	//std::cout << "position1: " << position1 << "------------------" << std::endl;
	m_helper.Update(m_bounds.xMax);
	//Vector position2 = m_helper.GetCurrentPosition();
	...
	double rd = m_normalDirection->GetValue();				//m_normalDirection主程序已设置
	...
	m_Direction = m_alpha * m_Direction + one_minus_alpha * m_meanDirection + sqrt_alpha * rd;		//m_alpha和m_meanDirection主程序已设置为0，sqrt_alpha为1，m_Direction=rd。如果超出范围会改变m_meanDirection值
	m_Pitch = m_alpha * m_Pitch + one_minus_alpha * m_meanPitch + sqrt_alpha * rp;
	//std::cout << "m_Direction: " << m_Direction << " " << m_alpha << " " << "rd: " << rd << std::endl;
	...
	//std::cout << "cosDircosPit: " << cosDir << " " << cosPit << " " << "m_Velocity: " << m_Velocity << std::endl;
	double sinDir = std::sin(m_Direction);
	double sinPit = std::sin(m_Pitch);
	double vx = m_Velocity * cosDir * cosPit;					//准备前进的位置
...
void GaussMarkovMobilityModel::DoWalk(Time delayLeft) {
	...
	//std::cout << "position3: " << position << "------------------" << nextPosition << std::endl;
	if (nextPosition.x > m_bounds.xMax) {
		nextPosition.x = nextPosition.x - m_bounds.xMax;
	}
	...
	if (m_bounds.IsInside(nextPosition)) {
		...
		if (nextPosition.x > m_bounds.xMax) {
			speed.x = -speed.x;							//x转向是用的m_meanDirection，这里没起作用
			//m_meanDirection = M_PI - m_meanDirection;				//注释，否则到达最大x之后会返回
		}
		if (nextPosition.x < m_bounds.xMin) {
			speed.x = -speed.x;
...
void GaussMarkovMobilityModel::DoDispose(void) {
	// chain up
	MobilityModel::DoDispose();
}

Vector GaussMarkovMobilityModel::DoGetPosition(void) const {
	m_helper.Update();
...
Vector GaussMarkovMobilityModel::DoGetVelocity(void) const {
