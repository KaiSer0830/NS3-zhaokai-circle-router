...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        result1(j) = (length(dataRx1) / length(dataTx1)) + result1(j);  
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fclose(fid2);
        dataTx1 = [C2{3}];                         %%取出第3个元素
        Variance1(j) = ((length(dataRx1) / length(dataTx1)) - result1(j))^2 + Variance1(j);
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        result2(j) = (length(dataRx2) / length(dataTx2)) + result2(j);  
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fclose(fid4);
        dataTx2 = [C4{3}];                         %%取出第3个元素
        Variance2(j) = ((length(dataRx2) / length(dataTx2)) - result2(j))^2 + Variance2(j);
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        result3(j) = (length(dataRx3) / length(dataTx3)) + result3(j); 
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fclose(fid6);
        dataTx3 = [C6{3}];                         %%取出第3个元素
        Variance3(j) = ((length(dataRx3) / length(dataTx3)) - result3(j))^2 + Variance3(j);
    end
end
