...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fid2 = fopen(file2);
        C2 = textscan(fid2, '%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid2);
        dataTx1 = [C2{2}];                         %%取出第3个元素
        result1(j) = ((length(dataRx1) + length(dataTx1)) * 100 / duration) + result1(j);  
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fclose(fid2);
        dataTx1 = [C2{2}];                         %%取出第3个元素
        Variance1(j) = (((length(dataRx1) + length(dataTx1)) * 100 / duration) - result1(j))^2 + Variance1(j);
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fid4 = fopen(file4);
        C4 = textscan(fid4, '%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid4);
        dataTx2 = [C4{2}];                         %%取出第3个元素
        result2(j) = ((length(dataRx2) + length(dataTx2)) * 100 / duration) + result2(j); 
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        fclose(fid4);
        dataTx2 = [C4{2}];                         %%取出第3个元素
        Variance2(j) = (((length(dataRx2) + length(dataTx2)) * 100 / duration) - result2(j))^2 + Variance2(j);
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        file6 = ['D:\professional_install\matlab\bin\project_files\opport_circle_zhao\log', num2str(i),'\opport_phyTx', num2str(j), '.log'];
        fid6 = fopen(file6);
        C6 = textscan(fid6, '%f%f');          %%其中fid为fopen命令返回的文件标识符，format实际上就是一个字符串变量，表示读取数据及数据转换的规则。
        fclose(fid6);
        temp = [C6{1}];                         %%取出第1个元素
        idx1 = find(temp(:,1)~=0);               %%取非零列元素
        dataTx3 = temp(idx1,:);
        idx2 = find(temp(:,1)==0);               %%取非零列元素
        dataTx4 = temp(idx2,:);
        result3(j) = (((length(dataRx3) + length(dataTx3)) * 100 + length(dataTx4) * 30) / duration) + result3(j); 
    end
end
...
for i = 1:filesNum
    for j=1:nodeNum
        ...
        idx2 = find(temp(:,1)==0);               %%取非零列元素
        dataTx4 = temp(idx2,:);
        Variance3(j) = ((((length(dataRx3) + length(dataTx3)) * 100 + length(dataTx4) * 30) / duration) - result3(j))^2 + Variance3(j); 
    end
end
